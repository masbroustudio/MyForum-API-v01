...
describe("a CommentDetails entities", () => {
	...
	it("should throw error when payload did not meet data type specification", () => {
		...
		const payload = {
			...
			username: "user-123",
			date: "2023-01-01T12:00:00.000Z",
			like_count: true, // invalid type, should be number
	...
	it("should create CommentDetails object with expected props", () => {
		...
		// Action
		const { id, username, date, content, replies, likeCount } = new CommentDetails(
			payload,
			mockReplies
		);
	...
	it("should create CommentDetails object with expected props if replies is not provided", () => {
		...
		// Action
		const { id, username, date, content, replies, likeCount } = new CommentDetails(payload);
